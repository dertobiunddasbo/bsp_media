...
import { getSectionContent, saveSectionContent, defaultWhyUsData } from '@/lib/api'
...
export default function WhyUs({ pageSlug = 'home' }: WhyUsProps) {
  ...
  const handleSave = async (newData: WhyUsData) => {
    try {
      const result = await saveSectionContent('whyus', newData, pageSlug)
      if (result.success) {
        ...
        console.error('Save failed for whyus section:', result.error)
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unbekannter Fehler'
      console.error('Error saving whyus:', errorMessage)
